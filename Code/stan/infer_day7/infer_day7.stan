//saved as single_beta.stan

functions{
  real[] single_beta(real t, real[] B, real[] theta, real[] x_r, int[] x_i){
  real dXdt[4];
  dXdt[1] = (x_r[4]/x_r[1])*B[1]*(1-((B[1] + x_r[8]*B[2])/B[3]));
  dXdt[2] = (x_r[5]/x_r[2])*B[2]*(1-((B[2] + x_r[8]*B[1])/B[4]));
    if (t < x_r[7]){
      dXdt[3] = 0;
      dXdt[4] = 0;
    } else {
...
data {
  ...
  real<lower=0> B0[2]; //initial states of B at 14 days
  ...
  real beta; //competition parameter
  real rel_tol;
  real abs_tol;
  real max_steps;
}
...
model {
  ...
  theta[1] ~ normal(0,1);
  theta[2] ~ normal(0,1);
  B[1] = (B0[1]/2) + (B0[1]/2)*theta[1];
  B[2] = (B0[2]/2) + (B0[2]/2)*theta[2];
